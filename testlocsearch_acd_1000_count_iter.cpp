...
//    lattice::PairPotentialProblem uprob(lattice::ljpotent, length, atoms);
	...
	myfile.open ("acd_1000_local_min.txt");
	myfile2.open("acd_1000_conf.txt");
	//myfile_time.open("acd_20_time.txt");
	myfile << "Local minimums \n nv,f\n";
	myfile2 << "Configurations \n f()\n";
	int times, timed, time2;
...
int seconds = std::chrono::duration_cast<std::chrono::seconds>(end - begin).count();
	//std::cout << iter << ". " << times << "\n";
	myfile_time.open("acd_20_time.txt", std::ios_base::app);
	std::cout << iter << " point from " << npoints << ". It took: " << seconds << " sec. \r";
	std::cout.flush(); 
	myfile_time << iter << "," << seconds << "\n";
	myfile_time.close();
